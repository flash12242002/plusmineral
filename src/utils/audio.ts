...
/**
 * Synthesizes an ultra-pure, comforting, high-frequency crystal-chime (水晶磬音) sound
 * for tactile feedback when switching tabs or clicking items.
 */
export function playCrystalChime(): void {
  try {
    ...
    const now = ctx.currentTime;
    // Standard pure, crystalline solfeggio frequency (528 Hz - transformation & peace)
    const baseFreq = 528;

    const components = [
      { ratio: 1.0,  gain: 0.18, decay: 0.8 },
      { ratio: 1.5,  gain: 0.08, decay: 0.6 },
      { ratio: 2.0,  gain: 0.05, decay: 0.4 },
      { ratio: 3.0,  gain: 0.03, decay: 0.25 }
    ];

    const masterGain = ctx.createGain();
    masterGain.gain.setValueAtTime(0, now);
    masterGain.gain.linearRampToValueAtTime(0.4, now + 0.015); // Fast crisp attack
    masterGain.gain.exponentialRampToValueAtTime(0.001, now + 1.2); // Zen fade-out
    masterGain.connect(ctx.destination);

    const activeNodes: (OscillatorNode | GainNode)[] = [masterGain];
    ...
    components.forEach((comp) => {
      ...
      oscGain.gain.setValueAtTime(0, now);
      oscGain.gain.linearRampToValueAtTime(comp.gain, now + 0.015);
      oscGain.gain.exponentialRampToValueAtTime(0.0001, now + comp.decay);
